...
For development with automatic builds and browser refresh:
#+BEGIN_SRC sh
uv run hotreload.py
#+END_SRC

The hot reload system automatically runs an initial build, runs `./build.sh` when files change, and serves the site via HTTP server at =http://localhost:8000= with automatic browser refresh. No flags needed!
...
Test change 2 for verification
Test change for public directory removal
